...
const initialNodes = [
  ...
  },
  {
    ...
    data: {
      ...
      subtitle: '全社管理リポジトリ  ★',
      items: [
        '/repos.yml  （管理対象リポジトリ一覧）',
        '/members  （メンバー・組織情報）',
        '/manuals  /knowledge  /labels',
        '/cli  /mcp  （ツール群）',
      ],
    },
  },
  {
    ...
    data: {
      ...
      items: [
        'pj init  → リポジトリ初期化 & mgmt-hub 登録',
        'pj use <repo>  （ローカルで管理対象を追加）',
        '朝会サポート・工数入力',
        '議事録 → Issue 自動生成',
      ],
    },
  },
  {
    ...
    data: {
      ...
      items: [
        'Issues / Milestones / Projects v2 操作',
        'gantt.yml 読み取り・更新',
        '複数リポジトリを横断してクエリ',
  ...
  // ── ③ 案件リポジトリ（独立 init 可能） ─────────────────────
  {
    ...
    data: {
      ...
      items: [
        '.pj-workhub/config.yml  （設定・ブランチ戦略）',
        '.pj-workhub/gantt.yml  （ガントチャート定義）',
        'Milestones  （フェーズ・期間管理）',
        'Issues / Projects v2  （タスク管理）',
      ],
    },
  },
  {
